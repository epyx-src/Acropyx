...
/**
 * Read the HTTP request content as an UTF-8 string and call back a function when finished
 * @param request HTTP request
 * @param response HTTP response
 * @param fn call back function ( content )
 */
var readRequestContent = function(request, response, fn) {
	if ( !fn ) {
		// no function, we do not need to handle the content read...
		util.warning("Reading JSON request content without a callback function");
	...
	request.on('end', function() {
		try {
			...
			if (response.finished == false) {
				// check if fn already answer the request
				response.writeHead(200);
				response.end("success", 'utf-8');
			}
		} catch (error) {
			response.writeHead(500);
			response.end("Error : "+error, 'utf-8');
		}
	});
};
 ...
 * @returns
 */
var testHttpRequest = function( handlers,end ) {
...
//-- Exports --
module.exports = {
	readRequestContent : readRequestContent
};